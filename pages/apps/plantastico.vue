<template>
  <div class="d-flex flex-column align-content-center">
    <div class=" pt-2 d-flex flex-column align-items-center justify-content-center">
      <h2>Plantastico</h2>
      <img :src="'/folio/plantastico-logo.png'" class="img-fluid" style="max-height:250px" />
      <p class="pt-2">
        Manage your basic plant care needs with simple reminders.
      </p>
    </div>

    <div class=" pt-4 d-flex align-items-center justify-content-center">
      <div class="card">
        <div class="card-body pt-2 d-flex flex-column align-items-center justify-content-center">
          <h4>Privacy Policy</h4>
          <p>The Plantastico iOS App does not collect any data. All recorded data is stored on the user's device and can be deleted with the app at any time.</p>
        </div>
      </div>
    </div>

    <div class=" pt-2 d-flex align-items-center justify-content-center">
      <a href="mailto:dsdev994@gmail.com" class="btn btn-pill folio-button">
        <img class="folio-button-icon" :src="'/folio/contact.svg'" alt="Icon of a person waving.">
        Email Support
      </a>
    </div>
  </div>
</template>

  <style lang="scss">
  @use '@/assets/styles/main.scss' as main;
  </style>
